...
ViewFinderQt::ViewFinderQt(QWidget* parent) : QWidget(parent), place_(rect()), buffer_(nullptr) {
...
auto ViewFinderQt::nativeFormats() const -> const QList<libcamera::PixelFormat>& {
  static const QList<libcamera::PixelFormat> formats = ::nativeFormats.keys();
  return formats;
}

auto ViewFinderQt::setFormat(const libcamera::PixelFormat& format, const QSize& size,
                             [[maybe_unused]] const libcamera::ColorSpace& colorSpace,
                             unsigned int stride) -> int {
  image_ = QImage();
...
void ViewFinderQt::render(libcamera::FrameBuffer* buffer, Image* image) {
...

  {
    ...
    if (::nativeFormats.contains(format_)) {
      ...
      image_ = QImage(image->data(0).data(), size_.width(), size_.height(), size / size_.height(),
...
auto ViewFinderQt::getCurrentImage() -> QImage {
  QMutexLocker locker(&mutex_);
...
auto ViewFinderQt::sizeHint() const -> QSize {
