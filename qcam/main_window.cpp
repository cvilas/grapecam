...
public:
  enum PlugEvent { HotPlug, HotUnplug };
  ...
  auto hotplugEvent() -> PlugEvent const {
    return plugEvent_;
  }
...
MainWindow::MainWindow(CameraManager* cm, const OptionsParser::Options& options)
  ...
  if (renderType == "qt") {
    ...
    connect(viewfinder, &ViewFinderQt::renderComplete, this, &MainWindow::renderComplete);
    viewfinder_ = viewfinder;
    setCentralWidget(viewfinder);
#ifndef QT_NO_OPENGL
  } else if (renderType == "gles") {
    ViewFinderGL* viewfinder = new ViewFinderGL(this);
    connect(viewfinder, &ViewFinderGL::renderComplete, this, &MainWindow::renderComplete);
    viewfinder_ = viewfinder;
    setCentralWidget(viewfinder);
#endif
  } else {
    qWarning() << "Invalid render type" << QString::fromStdString(renderType);
    quit();
    return;
  }
...
auto MainWindow::event(QEvent* e) -> bool {
...
auto MainWindow::createToolbars() -> int {
  QAction* action;
  ...
  /* Quit action. */
  action =
      toolbar_->addAction(QIcon::fromTheme("application-exit", QIcon(":x-circle.svg")), "Quit");
  action->setShortcut(QKeySequence::Quit);
  connect(action, &QAction::triggered, this, &MainWindow::quit);

  /* Camera selector. */
  cameraSelectButton_ = new QPushButton;
  connect(cameraSelectButton_, &QPushButton::clicked, this, &MainWindow::switchCamera);
  ...
  /* Start/Stop action. */
  iconPlay_ = QIcon::fromTheme("media-playback-start", QIcon(":play-circle.svg"));
  iconStop_ = QIcon::fromTheme("media-playback-stop", QIcon(":stop-circle.svg"));
  ...
  /* Save As... action. */
  action =
      toolbar_->addAction(QIcon::fromTheme("document-save-as", QIcon(":save.svg")), "Save As...");
...
auto MainWindow::chooseCamera() -> std::string {
...
auto MainWindow::openCamera() -> int {
 ...
 */
auto MainWindow::startCapture() -> int {
  ...
  switch (roles.size()) {
    case 1:
      ...
      break;
    case 2:
      if (roles[0] != StreamRole::Viewfinder || roles[1] != StreamRole::Raw) {
  ...
  for (const PixelFormat& format : viewfinder_->nativeFormats()) {
    auto match = std::find_if(formats.begin(), formats.end(),
                              [&](const PixelFormat& f) { return f == format; });
  ...
  if (validation == CameraConfiguration::Adjusted)
    qInfo() << "Stream configuration adjusted to " << vfConfig.toString().c_str();

  auto ret = camera_->configure(config_.get());
  if (ret < 0) {
   ...
   */
  ret =
      viewfinder_->setFormat(vfConfig.pixelFormat, QSize(vfConfig.size.width, vfConfig.size.height),
...
void MainWindow::addCamera(std::shared_ptr<Camera> camera) {
  ...
  QCoreApplication::postEvent(this, new HotplugEvent(std::move(camera), HotplugEvent::HotPlug));
}

void MainWindow::removeCamera(std::shared_ptr<Camera> camera) {
  qInfo() << "Removing camera:" << camera->id().c_str();
  QCoreApplication::postEvent(this, new HotplugEvent(std::move(camera), HotplugEvent::HotUnplug));
}
...
void MainWindow::processRaw(FrameBuffer* buffer, [[maybe_unused]] const ControlList& metadata) {
...
void MainWindow::processViewfinder(FrameBuffer* buffer) {
  ...
  qDebug().noquote() << QString("seq: %1").arg(metadata.sequence, 6, 10, QLatin1Char('0'))
                     << "bytesused: {" << bytesused.join(", ")
                     << "} timestamp:" << metadata.timestamp << "fps:" << Qt::fixed
                     << qSetRealNumberPrecision(2) << fps;
...
auto MainWindow::queueRequest(Request* request) -> int {
