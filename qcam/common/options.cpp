...
template <typename T>
bool OptionsBase<T>::parseValue(const T& opt, const Option& option, const char* arg) {
  OptionValue value;
 ...
 * occurred.
 */
bool KeyValueParser::addOption(const char* name, OptionType type, const char* help,
                               OptionArgument argument) {
  if (!name)
    return false;
  if (!help || help[0] == '\0')
  ...
  optionsMap_[name] =
      Option({ 0, type, name, argument, nullptr, help, nullptr, false, nullptr, {} });
  return true;
}
...
KeyValueParser::Options KeyValueParser::parse(const char* arguments) {
  ...
  for (const char* pair = arguments; *arguments != '\0'; pair = arguments) {
    ...
    if (!options.parseValue(key, option, value.c_str())) {
      std::cerr << "Failed to parse '" << value << "' as " << option.typeName() << " for option "
                << key << std::endl;
      return options;
    }
  }
 ...
 */
OptionValue::OptionValue(const char* value) : type_(ValueString), integer_(0), string_(value) {
}
 ...
 * occurred.
 */
bool OptionsParser::addOption(int opt, OptionType type, const char* help, const char* name,
                              OptionArgument argument, const char* argumentName, bool array,
                              int parent) {
  /*
  ...
  if (parent) {
    ...
    parentOpt->children.push_back(
        { opt, type, name, argument, argumentName, help, nullptr, array, parentOpt, {} });
    option = &parentOpt->children.back();
  } else {
    options_.push_back(
        { opt, type, name, argument, argumentName, help, nullptr, array, nullptr, {} });
 ...
 * occurred.
 */
bool OptionsParser::addOption(int opt, KeyValueParser* parser, const char* help, const char* name,
                              bool array, int parent) {
  if (!addOption(opt, OptionKeyValue, help, name, ArgumentRequired, "key=value[,key=value,...]",
                 array, parent))
    return false;
...
OptionsParser::Options OptionsParser::parse(int argc, char** argv) {
  ...
  while (true) {
    int c = getopt_long(argc, argv, shortOptions.data(), longOptions.data(), nullptr);
  ...
  if (optind < argc) {
    std::cerr << "Invalid non-option argument '" << argv[optind] << "'" << std::endl;
    usage();
    return options;
  }
...
void OptionsParser::usageOptions(const std::list<Option>& options, unsigned int indent) {
  ...
  for (const Option* option : parentOptions) {
    std::cerr << std::endl
              << "Options valid in the context of " << option->optionName() << ":" << std::endl;
    usageOptions(option->children, indent);
  }
}

std::tuple<OptionsParser::Options*, const Option*> OptionsParser::childOption(const Option* parent,
                                                                              Options* options) {
  /*
...
bool OptionsParser::parseValue(const Option& option, const char* arg, Options* options) {
  const Option* error;

  std::tie(options, error) = childOption(option.parent, options);
  if (error) {
    std::cerr << "Option " << option.optionName() << " requires a " << error->optionName()
              << " context" << std::endl;
    return false;
  }
