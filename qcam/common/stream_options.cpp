...
StreamKeyValueParser::StreamKeyValueParser() {
  addOption("role", OptionString, "Role for the stream (viewfinder, video, still, raw)",
            ArgumentRequired);
  ...
  addOption("colorspace", OptionString, "Color space", ArgumentRequired);
}
...
KeyValueParser::Options StreamKeyValueParser::parse(const char* arguments) {
  ...
  if (options.valid() && options.isSet("role") && !parseRole(options)) {
    std::cerr << "Unknown stream role " << options["role"].toString() << std::endl;
    options.invalidate();
  }
...
auto StreamKeyValueParser::roles(const OptionValue& values) -> std::vector<StreamRole> {
...
auto StreamKeyValueParser::updateConfiguration(CameraConfiguration* config,
                                               const OptionValue& values) -> int {
  if (!config) {
  ...
  if (config->size() != streamParameters.size()) {
    std::cerr << "Number of streams in configuration " << config->size()
              << " does not match number of streams parsed " << streamParameters.size()
              << std::endl;
    return -EINVAL;
  }
...
auto StreamKeyValueParser::parseRole(const KeyValueParser::Options& options)
