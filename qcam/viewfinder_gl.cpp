...
ViewFinderGL::ViewFinderGL(QWidget* parent)
  ...
  , colorSpace_(libcamera::ColorSpace::Raw)
...
auto ViewFinderGL::nativeFormats() const -> const QList<libcamera::PixelFormat>& {
  return supportedFormats;
}

auto ViewFinderGL::setFormat(const libcamera::PixelFormat& format, const QSize& size,
                             const libcamera::ColorSpace& colorSpace, unsigned int stride) -> int {
...
auto ViewFinderGL::getCurrentImage() -> QImage {
...
auto ViewFinderGL::selectFormat(const libcamera::PixelFormat& format) -> bool {
  bool ret = true;
...
void ViewFinderGL::selectColorSpace(const libcamera::ColorSpace& colorSpace) {
  ...
  switch (colorSpace.ycbcrEncoding) {
    ...
    case libcamera::ColorSpace::YcbcrEncoding::Rec601:
      yuv2rgb = {
        1.0000, 1.0000, 1.0000, 0.0000, -0.3441, 1.7720, 1.4020, -0.7141, 0.0000,
      };
      break;

    case libcamera::ColorSpace::YcbcrEncoding::Rec709:
      yuv2rgb = {
        1.0000, 1.0000, 1.0000, 0.0000, -0.1873, 1.8856, 1.5748, -0.4681, 0.0000,
      };
      break;

    case libcamera::ColorSpace::YcbcrEncoding::Rec2020:
      yuv2rgb = {
        1.0000, 1.0000, 1.0000, 0.0000, -0.1646, 1.8814, 1.4746, -0.5714, 0.0000,
      };
      break;
  }
  ...
  fragmentShaderDefines_.append(QString("#define YUV2RGB_Y_OFFSET %1").arg(offset, 0, 'f', 1));
}

auto ViewFinderGL::createVertexShader() -> bool {
...
auto ViewFinderGL::createFragmentShader() -> bool {
  int attributeVertex;
  ...
  shaderProgram_.setAttributeBuffer(attributeTexture, GL_FLOAT, 8 * sizeof(GLfloat), 2,
                                    2 * sizeof(GLfloat));
...
void ViewFinderGL::doRender() {
  ...
  switch (format_) {
    ...
    case libcamera::formats::NV42:
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, stride_, size_.height(), 0, GL_LUMINANCE,
                   GL_UNSIGNED_BYTE, image_->data(0).data());
      shaderProgram_.setUniformValue(textureUniformY_, 0);

      /* Activate texture UV/VU */
      glActiveTexture(GL_TEXTURE1);
      configureTexture(*textures_[1]);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, stride_ / horzSubSample_,
                   size_.height() / vertSubSample_, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,
                   image_->data(1).data());
    ...
    case libcamera::formats::YUV420:
      ...
      glActiveTexture(GL_TEXTURE1);
      configureTexture(*textures_[1]);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, stride_ / horzSubSample_,
                   size_.height() / vertSubSample_, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
                   image_->data(1).data());
      shaderProgram_.setUniformValue(textureUniformU_, 1);
    ...
    case libcamera::formats::YVYU:
      ...
      glActiveTexture(GL_TEXTURE0);
      configureTexture(*textures_[0]);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, stride_ / 4, size_.height(), 0, GL_RGBA,
                   GL_UNSIGNED_BYTE, image_->data(0).data());
      shaderProgram_.setUniformValue(textureUniformY_, 0);
       ...
       */
      shaderProgram_.setUniformValue(textureUniformStep_, 1.0f / (size_.width() / 2 - 1),
                                     1.0f /* not used */);
    ...
    case libcamera::formats::RGB888:
      glActiveTexture(GL_TEXTURE0);
      configureTexture(*textures_[0]);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, stride_ / 3, size_.height(), 0, GL_RGB,
                   GL_UNSIGNED_BYTE, image_->data(0).data());
      shaderProgram_.setUniformValue(textureUniformY_, 0);
    ...
    case libcamera::formats::SRGGB12_CSI2P:
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, stride_, size_.height(), 0, GL_LUMINANCE,
                   GL_UNSIGNED_BYTE, image_->data(0).data());
      shaderProgram_.setUniformValue(textureUniformY_, 0);
      shaderProgram_.setUniformValue(textureUniformBayerFirstRed_, firstRed_);
      shaderProgram_.setUniformValue(textureUniformSize_, size_.width(), /* in pixels */
                                     size_.height());
      shaderProgram_.setUniformValue(textureUniformStep_, 1.0f / (stride_ - 1),
                                     1.0f / (size_.height() - 1));
...
auto ViewFinderGL::sizeHint() const -> QSize {
